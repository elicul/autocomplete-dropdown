...
@Component({
  selector: 'p-autocomplete-dropdown-item',
  template: `
    <li
...
export class AutocompleteDropdownItem {
            ...
            >
              ...
              <ng-template #virtualScrollList>
                <cdk-virtual-scroll-viewport
                  ...
                  [itemSize]="itemSize"
                  *ngIf="
                    virtualScroll && optionsToDisplay && optionsToDisplay.length
                  "
                >
                  <ng-container
                    *cdkVirtualFor="
...
export class AutocompleteDropdown
  ...
  set options(val: any[]) {
    let opts = this.optionLabel
      ? ObjectUtils.generateSelectItems(val, this.optionLabel)
      : val;
    this._options = opts;
  ...
  onMouseclick(event) {
    ...
    if (!this.itemClick && !this.clearClick) {
      ...
      if (this.overlayVisible) this.hide(event);
      else this.show();
  ...
  appendOverlay() {
    if (this.appendTo) {
      ...
      this.overlay.style.minWidth =
        DomHandler.getWidth(this.containerViewChild.nativeElement) + 'px';
    }
  }
        ...
        if (!this.overlayVisible && event.altKey) {
          ...
          if (this.group) {
            ...
            if (selectedItemIndex !== -1) {
              ...
              if (
                nextItemIndex <
                this.optionsToDisplay[selectedItemIndex.groupIndex].items.length
              ) {
                this.selectItem(
                  event,
                  this.optionsToDisplay[selectedItemIndex.groupIndex].items[
                    nextItemIndex
                  ]
                );
                this.selectedOptionUpdated = true;
            ...
            let selectedItemIndex = this.selectedOption
              ...
              : -1;
            let nextEnabledOption = this.findNextEnabledOption(
              selectedItemIndex
            );
            if (nextEnabledOption) {
        ...
        if (this.group) {
          ...
          if (selectedItemIndex !== -1) {
            ...
            if (prevItemIndex >= 0) {
              ...
              let prevGroup = this.optionsToDisplay[
    ...
    if (this.searchValue) {
      for (let i = index.groupIndex; i < this.optionsToDisplay.length; i++) {
        for (
          ...
          if (
            opt.label.toLowerCase().startsWith(this.searchValue.toLowerCase())
          ) {
            return opt;
          }
        }
  ...
  activateFilter() {
    ...
    if (this.options && this.options.length) {
      if (this.group) {
        ...
        this.optionsToDisplay = FilterUtils.filter(
          this.options,
          searchFields,
          this.filterValue,
          this.filterMatchMode
